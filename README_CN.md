...
- **专家会诊**: 启动 WebUI 后，点击左侧“会诊”图标，即可体验多模型并行分析。
...
2.  **专家会诊模式**: 【新功能】支持多个 AI 模型（如 Gemini + DeepSeek）并行分析，对比差异并生成共识结论。
3.  **混合配置系统**: 【新功能】支持从环境变量（适合 CI/CD）或数据库（适合服务器热更新）加载配置。
4.  **决策仪表盘**: 生成包含买卖建议、支撑压力位、风险提示的直观日报。
5.  **多渠道推送**: 支持飞书、微信、Telegram 等多种通知方式。
6.  **大盘复盘**: 自动总结每日市场行情。
7.  **Web 管理**: 提供可视化的管理界面。
